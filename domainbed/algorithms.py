...
class Classifier(Algorithm):
    """
    Simplified Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, env, hparams):
        super(Classifier, self).__init__(input_shape, num_classes, env,
                                  hparams)
        self.env = env
    ...
    def update(self, minibatches, unlabeled=None):
        p = self.predict(minibatches[0])
        loss = F.cross_entropy(p, minibatches[1])

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        return {'env{}_loss'.format(self.env): loss.item()}

    def predict(self, x):
        return self.network(x)

    def propensity_score(self, x):
        return F.softmax(self.network(x), -1)
