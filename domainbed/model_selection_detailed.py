...
class SelectionMethod:
    ...
    def sweep_acc(self, records):
        ...
        if len(_hparams_accs):
            return {"sweep_acc": _hparams_accs[0][0]['test_acc'], 
                    'chosen_record': _hparams_accs[0][0]['chosen_record']}
        else:
            return None

class OracleSelectionMethod(SelectionMethod):
    """Like Selection method which picks argmax(mean(env_out_acc for env in test_envs)) across all hparams
    ...
    def run_acc(self, run_records):
        ...
        chosen_record = run_records.sorted(lambda r: r['env{}_out_acc'.format(test_env)])[-1]
        val_acc = chosen_record['env{}_out_acc'.format(test_env)]
        test_acc = chosen_record['env{}_in_acc'.format(test_env)]
        return {
            'val_acc':  val_acc,
            'test_acc': test_acc,
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, record):
        ...
        test_in_acc_keys = ['env{}_in_acc'.format(test_env) 
                            for test_env in test_envs]
        return {
            'val_acc': np.mean([record[key] for key in val_env_keys]),
            'test_acc': np.mean([record[key] for key in test_in_acc_keys]),
